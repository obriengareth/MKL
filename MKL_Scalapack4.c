...
mpicc -o MKL_Scalapack4 MKL_Scalapack4.c -mkl -lmkl_blacs_intelmpi_lp64 -lmkl_scalapack_lp64
mpirun -np 4 ./MKL_Scalapack3
...
int main(int argc ,char *argv[])
{
  ...
  //    print_matrix("", mla, nrhs, bl, mla );
	...
	psgels_("N" , &mg , &mg , &nrhs , Al , &one, &one, descal, bl, &one,&one,descbl, &wkopt, &lwork, &info ); 
	lwork = (int)wkopt;
	work = (float*)malloc( lwork*sizeof(float) );
	// Solve the equations A*X = B 
	psgels_("N" , &mg , &mg , &nrhs , Al , &one, &one,descal,bl, &one,&one,descbl, work, &lwork, &info ); 
	//printf("info=%d \n",info);
	}
    else
    {
        int lwork=-1;  float wkopt;   float* work;      
	psgels_("N" , &mg , &ng , &nrhs , Al , &one, &one, descal, bl, &one,&one,descbl, &wkopt, &lwork, &info ); 
	lwork = (int)wkopt;
	work = (float*)malloc( lwork*sizeof(float) );
	// Solve the equations A*X = B 
	psgels_("N" , &mg , &ng , &nrhs , Al , &one, &one,descal,bl, &one,&one,descbl, work, &lwork, &info ); 
	//printf("info=%d \n",info);
    }
