...
mpicc -o MKL_FGMRES1 MKL_FGMRES1.c -mkl -lmkl_blacs_intelmpi_lp64 -lmkl_scalapack_lp64
mpirun -np 4 ./MKL_FGMRES1
...
int main(int argc, char *argv[])
{
	...
	//	printf("b_2norm=%f\n",b_2norm);
		...
		/*The routine dfgmres_init initializes the solver. After initialization all subsequent invocations of 
		Intel MKL RCI FGMRES routines use the values of all parameters that are returned by dfgmres_init. 
...
ONE:
	...
	if (RCI_flag == 2)
	{
		/* Request to the dfgmres_get routine to put the solution into b[N] via ipar[12]
		   --------------------------------------------------------------------------------
		Beware that the call to dfgmres_get routine with ipar[12]=0 at this
